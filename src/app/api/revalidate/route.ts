 ...
 * 13. Secret: Set to the same value as SANITY_REVALIDATE_SECRET (create a random secret if you haven't yet, for example by running `Math.random().toString(36).slice(2) or node > require("crypto").randomBytes(32).toString("bas64url")` in your console)
...
export async function POST(req: NextRequest) {
  try {
    ...
    // revalidate comp w/ tag
    // document from Sanity is returned, limit info based on projection (eg. { _type })
